...
**스타일링**: `style` 속성 대신 Tailwind CSS 클래스를 최우선으로 사용할 것.

**네이밍**: 변수·함수·컴포넌트명은 역할이 즉시 파악되는 명확하고 직관적인 이름으로 지을 것.

**코딩 전 사고**: 구현 전에 가정을 명시하고, 불확실하면 먼저 물어볼 것. 해석이 여러 가지라면 혼자 선택하지 말고 나열해서 확인할 것.

**단순함 우선**: 요청받은 것 이상은 구현하지 말 것. 한 번만 쓸 코드에 추상화 금지. "시니어 엔지니어가 보면 과하다고 할까?" 스스로 물어볼 것.
